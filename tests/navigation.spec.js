...
test.describe('Navigation Links', () => {
  ...
  test.describe('Hero Section Links', () => {
    test('What I Do button navigates to services page', async ({ page }) => {
      await page.goto('/')
    ...
    test('Get in Touch button has correct mailto link', async ({ page }) => {
      await page.goto('/')
      const link = page.locator('section').first().getByRole('link', { name: 'Get in Touch' })
      await expect(link).toHaveAttribute('href', 'mailto:halderm@arkadia-labs.io')
    })
  })

  test.describe('Home Page Content Links', () => {
    test('See the Full List button navigates to services page', async ({ page }) => {
      await page.goto('/')
      await page.getByRole('link', { name: 'See the Full List' }).click()
      await expect(page).toHaveURL('/services')
    })

    test('Get in Touch CTA button has correct mailto link', async ({ page }) => {
      await page.goto('/')
      const link = page.getByRole('main').getByRole('link', { name: 'Get in Touch' }).last()
      await expect(link).toHaveAttribute('href', 'mailto:halderm@arkadia-labs.io')
    })
  })
  ...
  test.describe('Services Page Links', () => {
    test('Get in Touch button has correct mailto link', async ({ page }) => {
      await page.goto('/services')
      const link = page.getByRole('main').getByRole('link', { name: 'Get in Touch' })
      await expect(link).toHaveAttribute('href', 'mailto:halderm@arkadia-labs.io')
    })
  })
})
